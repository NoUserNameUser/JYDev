...
deploys the digest-pinned image through AWS Systems Manager. No inbound SSH rule is required.
GHCR authentication, SSM/IAM setup, first-run instructions, and
...
- `AWS_DEPLOY_ROLE_ARN`: GitHub OIDC role allowed to invoke SSM on the production instance.

Configure these repository variables:

- `AWS_REGION`: region containing the EC2 instance.
- `EC2_INSTANCE_ID`: production EC2 instance ID.
...
On the server, keep the production `.env` in `DEPLOY_PATH`, owned by `DEPLOY_USER`. It should
contain the database, Payload, revalidation, CORS, Telegram, and Postgres values from
`.env.production.example`. SSM sends the current Compose and deployment script, updates `WEB_IMAGE`,
and runs the rollout as `DEPLOY_USER`.
